# car3_swarm 多车开发文档（2026-08-17）

> 三车入侵与围捕仿真开发全记录：需求、架构、关键设计决策、实测验证数据、问题与修复。

## 1. 项目目标与需求

在 `nesting_room`（内房间 5.95m 见方 + 四扇 1.2m 门 + 14m 方形外房间）中部署**三辆完全同构的 car3**，
模拟"入侵车 + 两辆围捕车"场景。多车仿真独立于单 car3 仿真，放在新包 `car3_swarm` 下，
**不改动单 car3 仿真（car3_control）的既有行为**。

需求映射：

| 需求 | 方案 | 落位 |
|---|---|---|
| 1.1 新工程包 | 新包 `car3_swarm`（launch/src/scripts/params/config） | 本包 |
| 1.2 三车同构 | 同一 URDF（xacro 参数化前缀）→ 三份 robot_description；同一套控制器/nav 参数 | §3.2 |
| 1.3 固定出生位姿 | launch 参数默认值固定（每次运行一致，除非用户改参数） | §3.3 |
| 1.4 话题/服务严格区分 | 每车 ROS 命名空间 `car0/car1/car2` + TF 帧前缀；接口全表 | §3.4 |
| 1.5 两版六脚本 | V1 键盘 ×3、V2 多点导航 ×3（actionlib 等结果，防目标覆盖） | §4 |
| 1.6 位置共享 | `car_state_broadcaster` 读 gazebo 真值 → 每车 `/carN/shared_pose`（Odometry） | §3.5 |
| 1.7 虚拟障碍物 | 每车一个 `virtual_obstacle_node`：把其他车的虚拟长方体注入本车雷达扫描 | §3.6 |
...
| `nav_to_pose_node` | 话题改相对名：`cmd_vel`/`amcl_pose`/`odom`/`move_base_simple/goal`；`scan_topic` 默认改 `scan` |

原 `car3.urdf` 不动，单 car3 仿真零影响。

### 3.2 参数化 URDF（`car3_control/urdf/car3.xacro`）

内容与 `car3.urdf` 完全一致，仅两处参数化：
...
| `/carN/odom` | Odometry | 发布 | gazebo 真值里程计 |
...
| `/carN/wheel_{lf,rf,lb,rb}_velocity_controller/command` | Float64 | 订阅 | 轮速指令（ros_control） |
...
- 任何节点订阅三个 `/carN/shared_pose` 即可知全车状态

### 3.6 虚拟障碍物（需求 1.7）——方案选型
...
20Hz set_model_state 跟随车。
**致命缺陷：自己车的 lidar 位于自己盒子的几何中心内部**——每条激光束在 ~0.2m 处就打在本车盒子上，
本车扫描变成一个 0.2m 的障碍环，真实环境完全被遮挡。这是物理本质（lidar 在盒子内部），
bitmask 无法区分"自己的盒子"。

**方案 B（雷达扫描注入，采用）**：每车一个 `virtual_obstacle_node`：

- 订阅本车真扫描 `/carN/scan` + **其他两车**的 `/carM/shared_pose`
- 对每条激光束做"射线 vs 他车虚拟长方体（0.45×0.38m，2D 矩形）"相交计算：
  若相交且入口距离 < 原量程 → 把该束量程覆写为入口距离
- 发布 `/carN/scan_filtered`；`nav_to_pose` 的 `scan_topic` 指向它（**AMCL 仍用真扫描**，定位不受影响）
...
实测 4 次启动中 1 次三车 scan 全无数据（话题已 advertise、订阅者已连接）。
根因未完全定位（缺失的 "Starting Laser Plugin" 日志行被证实是 rosout 丢行，非真因）。
缓解措施：9 个导航节点（3×AMCL + 3×virtual_obstacle + 3×nav_to_pose）加 launch-prefix，
**等本车 `/carN/scan` 出首帧再启动**（90s 上限，超时回退旧行为），消除 spawn 窗口内的订阅竞争。
...
- **防覆盖策略**：actionlib `SimpleActionClient(/carN/move_base)`，
...
| 阶段 | 内容 | 验证结果 |
|---|---|---|
| 1 | 包骨架 + 单车节点 ns 化 + car3.xacro | `catkin build` 通过；单 car3 原 launch 回归通过（倒车 1.06m 实测，行为不变） |
| 2 | `multi_car3_bringup.launch`：三车固定位姿出生 | `/carN/*` 话题齐全、TF 树三车隔离；键盘只动车 0 时其余两车 1e-7 级静止 |
| 3 | `multi_car3_nav.launch`：3×AMCL + 3×nav_to_pose | AMCL 收敛到各自出生点（误差 <5cm）、三车独立导航 |
| 4 | V1 三键盘脚本 | 话题与按键映射 headless 验证通过，终端实测由用户操作 |
| 5 | `car_state_broadcaster` | `/carN/shared_pose` 数值与 gazebo 真值一致、20Hz 实测 |
| 6 | `virtual_obstacle_node` | 注入实测：car0 扫向 car2 方向 raw 3.4m（墙）→ filtered 0.8m（虚拟盒）；**对驶避障实测**：0.3m 处触发右向横移逃逸（航向锁定、x 向停进）→ 绕过盒子（净距 ~0.5m）→ 恢复直行到达目标；car2 全程零位移（无物理碰撞） |
| 7 | V2 三点导航脚本 | 全流程三次跑通（9 个目标全部 SUCCEEDED，无覆盖）：入侵车穿北门入内房间，围捕车同步收缩至门内侧两翼 |
| 8 | GUI 演示（2026-08-17 追加） | `gui:=true` + RViz 多车配置（`config/multi_car3.rviz`）屏幕实跑：9/9 SUCCEEDED，最终阵型——入侵车 (-0.03, 0.81) 内房间中心，围捕车 (0.62, 0.34) / (-0.70, 0.31) 分守北门两翼 |
...
| 激光插件偶发静默 | 4 次启动中 1 次三车 scan 全无数据；缺失的 "Starting Laser Plugin" 日志行是 rosout 丢行，非真因 | 9 个 nav 节点加 launch-prefix：等本车 scan 出首帧再启动（90s 上限，超时回退） |
| 目标落在别车排斥场内 | 目标距他车虚拟盒 <0.6m（repulse_range）时，吸引-排斥平衡形成绕目标极限环，永不 settle（6.5s 周期） | 属预期行为（保护域生效）；**规划导航点时需与他车保持 >0.6m 距离** |
| GUI 演示首次启动 spawn 冲突 | 上一轮验证的仿真进程未清理干净（旧 rosmaster/gzserver 仍存活），新 launch 的 spawn 报 "entity already exists"、controller_spawner 报 "new node registered with same name" | 彻底清理残留进程后干净重启即恢复；**每次新开仿真前先确认无残留进程**（见使用文档 §8） |
...
- "Starting Laser Plugin" 日志行可能只有 2 条（rosout 丢行），以 scan 实际有数据为准
- 首次三车同时 spawn，gzserver 负载上升（地图为静态加载，无建图开销）

## 8. 文件清单

```
car3_swarm/
├── README.md                      # 旧版开发记录（含方案选型与实测数据）
├── 开发文档_2026-08-17.md          # 本文档
├── 使用文档.md                     # 使用说明（启动/脚本/接口/排障）
├── CMakeLists.txt / package.xml
├── launch/
│   ├── multi_car3_bringup.launch  # 世界+三车（V1 键盘用）
│   ├── multi_car3_nav.launch      # 完整导航栈（V2 导航用）
│   └── car_spawn.launch           # 单车 spawn 模板（含参数化 xacro 展开）
├── src/
│   ├── car_state_broadcaster.cpp  # 1.6 位置共享
│   └── virtual_obstacle_node.cpp  # 1.7 虚拟障碍物扫描注入
├── scripts/
│   ├── swarm_teleop.py            # V1 键盘公共逻辑
│   ├── swarm_waypoints.py         # V2 导航公共逻辑（防覆盖）
│   ├── keyboard_car0/1/2.py       # V1 三入口
│   ├── waypoints_car0/1/2.py      # V2 三入口（改导航点编辑这里）
│   └── record_trajectory.py       # 轨迹录制工具
├── config/
│   └── multi_car3.rviz            # RViz 多车视图（地图+三车 TF+三色扫描）
└── params/                        # 预留（当前复用 car3_control/params）
```
