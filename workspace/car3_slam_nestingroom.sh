#!/bin/bash
# nesting_room 手动建图启动脚本（在容器 xtdrone-dev 内运行）
# 用法:
#   docker exec -it xtdrone-dev bash /workspace/car3_slam_nestingroom.sh
# 一条命令启动 Gazebo(nesting_room) + car3 + gmapping + RViz + 键盘控制。
# 在启动终端里用键盘开车:
...
echo "==> [2/3] 启动 Gazebo(nesting_room) + gmapping + 键盘控制 + RViz"
roslaunch car3_control car3_slam_nestingroom.launch gui:=true
